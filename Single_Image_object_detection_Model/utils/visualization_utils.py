...
def intersects(xmin1,ymin1,xmax1,ymax1,xmin2,ymin2,xmax2,ymax2):
    
    print str(ymax2) + " > " + str(ymin1) + "and " + str(ymax2) +" <= " + str(ymax1) 
    
    if (int(float(ymax2)) > int(float(ymin1))) and (int(float(ymax2)) <= int(float(ymax1))) :
         print "Stage 1 True ymax2 > ymin1 and ymax2 <= ymax1"
         print str(xmin2) + " > " + str(xmin1) + "and " + str(xmin2) +" < " + str(xmax1) +"or"+str(xmax2)+" < "+str(xmax1)
         if ( ( int(float(xmin2)) > int(float(xmin1))) and (int(float(xmin2)) < int(float(xmax1)) ) ) or (int(float(xmax2)) < int(float(xmax1))):
             print "Stage 2 True xmin2 > xmin1 and xmin2 < xmax1 or xmax2 < xmax1"
             return True
    # if the other rectangle is containing the first rectangle.     
    elif(int(float(ymax2)) > int(float(ymin1))) and (int(float(ymax2)) >= int(float(ymax1))):
          if (( int(float(xmin2)) > int(float(xmin1))) and (int(float(xmin2)) < int(float(xmax1)) ) ) or (int(float(xmax2)) >= int(float(xmax1))):
    #return not (xmax1 < xmin2 or xmin1 > xmax2 or ymax1 < ymin2 or ymin1 > ymax2)
             print "The second rectangle engulfs the first one."
             return True
...
def draw_bounding_box_on_image(image,
  ...
  if use_normalized_coordinates:
    ...
    print "LEFT im_width xmin"
    print left
    print "RIGHT im_width xmax"
    print right
    print "TOP im_height ymin"
    print top
    print "Bottom im_height ymax"
    print bottom
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        object_lists.append(display_str)    
  ...
  people_count = 0;
  helmet_count =0;
  for key,value in box_to_display_str_map.iteritems():
      print "Dimensions"
      print key
      print "Category"
      print value[0]
      ...
      ymin = key[0] * im_height
      xmin = key[1] * im_width
      ymax = key[2] * im_height
      xmax = key[3] * im_width
      
      print ymin, xmin,ymax,xmax
      
      if 'motorcycle' in value[0]:
          
          motorcycle_count = motorcycle_count + 1
          keym = 'motorcycle' + str(motorcycle_count)+" "+ str(value)
          valm = str(ymin) +',' + str(xmin) + ','+ str(ymax) + ',' + str(xmax)
          print "valm........"
          print valm
          motorcycle_list[keym] = valm          
                           
      if 'person' in value[0]:
          
          people_count = people_count + 1
          keyp = 'people' + str(people_count) + " " + str(value)
          valp = str(ymin) + ',' + str(xmin) + ',' + str(ymax) + ',' + str(xmax)
          print "valp........"
          print valp
          people_list[keyp] =  valp
          
      if 'Helmet' in value[0]:
          
          helmet_count = helmet_count + 1
          keyh = 'Helmet' + str(helmet_count) + " " + str(value)
          valh = str(ymin) + ',' + str(xmin) + ',' + str(ymax) + ',' + str(xmax)
          print "valh........"
          print valh
          helmet_list[keyh] =  valh
  ...
  intersect_dict={}
  if(Phase==2):
     print "Starting Phase2 helmet intersection with people"
     
     if bool(overlp_people):
         for K,V in helmet_list.iteritems():
             for K1,V1 in overlp_people.iteritems():
                 
                print str(V.split(",")[1])+" "+ str(V.split(",")[0])+" "+str(V.split(",")[3])+" "+str(V.split(",")[2])+" "+ str(V1.split(",")[1])+ " "+ str(V1.split(",")[0])+ " "+str(V1.split(",")[3])+" "+ str(V1.split(",")[2])
                Overl=intersects(V.split(",")[1],V.split(",")[0],V.split(",")[3],V.split(",")[2],V1.split(",")[1],V1.split(",")[0],V1.split(",")[3],V1.split(",")[2])
              
                if(Overl):
                  
                  print " Helmet Intersects Person"
                  helmet_overlap = helmet_overlap + 1
                  
                  if K in intersect_dict:
                    intersect_dict[K] = str(intersect_dict[K]) + " " + K1
                  else:
                    intersect_dict[K] = K1         
                             
                else:
                  
                  print "Does not Intersect with Person"
  ...
  if(Phase==1):                
      ...
      print "Length"
      print len(motorcycle_list)
      lng = len(motorcycle_list)/2
      
      mklist = list(motorcycle_list.keys())
      
      lenk = len(mklist)
      
      motorcycle_list_tmp1 = copy.deepcopy(motorcycle_list)
      motorcycle_list_tmp2 = copy.deepcopy(motorcycle_list)
      
      l1 = lenk/2-1
      
      for key1 in mklist[:l1]:
          del motorcycle_list_tmp1[key1]
          
      for key2 in mklist[l1:]:
          del motorcycle_list_tmp2[key2]
      
      print "motorcycle List1"
      print motorcycle_list_tmp1
      print "motorcycle List2"
      print motorcycle_list_tmp2
      print "----------------"
      
      motointerest = 0
      if motorcycle_count > 1:
         
         print "In case of Multiple Motorcycles. Check for Duplicate detection." 
         for K,V in motorcycle_list_tmp1.iteritems():
           for K1,V1 in motorcycle_list_tmp2.iteritems():
               if (K!=K1):
                   print "Coordinates........."
                   #print V
                   #print V1
                   ...
                   if(Overl):
                      ...
                      print "Intersection of Motorcycles"
                      motointerest=motointerest+1
                      print "--------------------Modified Motorcycle List--------"
                        
         
      max_overlap=0    
      if (people_count >= 1)  and ( motorcycle_count >= 1 ) :
         print "Entering the Comparison."
         for K,V in motorcycle_list.iteritems():
               print "Coming Here."
               for K1,V1 in people_list.iteritems():
                  
                 print "V of the motorcycle" 
                 print V.split(",")[0]
                 print V.split(",")[1]
                 print V.split(",")[2]
                 print V.split(",")[3]
                 print "V of the People"
                 print V1.split(",")[0]
                 print V1.split(",")[1]
                 print V1.split(",")[2]
                 print V1.split(",")[3]
      ...
      print " Maximum Intersections Between People and Motorcycle"
      print max_overlap
      ...
      d = 'Intersection' + ":" + str(max_overlap)
      print d
  ...
  return image,object_lists,overlap_people,intersect_dict
